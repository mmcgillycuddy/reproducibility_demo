# Reproducibility Demo 

This repository contains the code and documentation for a reproducibility seminar demo analysing the Banana Quality Dataset from Kaggle.

### File structure
The following directories are included with this template.

* **data:** All data should go in here. This repo is configured to not track any
  of the content of this directory but please double check that you do not
  push any data to GitHub.
  
* **docs:** Documentation relevant to the project goes here. 

* **figures:** The figures of your analysis goes here.

* **outputs:** The output of your analysis goes here.

* **src:** Place your code files (R scripts etc.) in this directory. You may
  also want to add a README file to explain how to use the code if there is 
  more than one file.

